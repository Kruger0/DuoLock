	...
	if (abs(vel.x) > acel*_min_acel || abs(vel.y) > acel*_min_acel) {
		...
		if (dust_time < 1 || true) {
			...
			if (chance(30)) {
				...
				var _col = choose(#FFFFFF, #DDDDDD, #BBBBBB)
				if (place_meeting(x, y, obj_water)) _col = choose(#10d0ff, #2080C0, #10bFDF)
				if (place_meeting(x, y, obj_poison)) _col = choose(#10FF30, #20CF20, #20DF40)
