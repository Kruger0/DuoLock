...
function menu_draw() {
	...
	for (var i = 0; i < op_length; i++) {
		...
		// draw options
		scribble(_str).blend(_c, 1).transform(_xscal*_nscl, _yscal*_nscl).align(0, 1).draw(_xx, _yy)	
		...
		if (menu_level == 2 && i < 3) {
			...
			scribble(_volume).blend(_c, 1).transform(_xscal*_nscl, _yscal*_nscl).align(0, 1).draw(_slider_x + 190, _yy)
		...
		// arrow
		if (_selec) {
			var _arrow_scl = _nscl*0.8
			draw_sprite_ext(spr_arrow_r, 0, _xx - 16, _yy + 2, _arrow_scl, _arrow_scl, 0, -1, 1)
		}	
	}
}
